...
export async function GET() {
  try {
    const firstRes = await fetch(
      `${PEERINGDB_API}/fac?format=json&status=ok&limit=${PAGE_SIZE}&skip=0`,
    ...
    const pages = await Promise.allSettled(
      offsets.map(skip =>
        fetch(`${PEERINGDB_API}/fac?format=json&status=ok&limit=${PAGE_SIZE}&skip=${skip}`, {
          headers: HEADERS,
    ...
    const facilities = all
      ...
      .map(f => ({
        ...
        name: f.name || '',
        city: f.city || '',
        country: f.country || '',   // ISO 2-letter code e.g. "IN", "US"
